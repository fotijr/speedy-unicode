...
const getSelectedUnicode = (): UnicodeCharacter => {
  return characters.find(
    (c) => c.code === filteredResults[selectedRowIndex].code
  );
};
...
const getFilteredCharacters = (searchTerm: string) => {
  ...
  let results: ScoredResult[] = characters.reduce(
    (chars: ScoredResult[], char: UnicodeCharacter) => {
      let score = 1000;
  ...
  render(
    ...
    () => html`
      <table>
        <tbody>
          ${chars.map(
            (c, i) => html.for(c, c.code)`
            <tr id=${i} onclick=${() =>
              rowSelected(c, i)} oncontextmenu=${showContextMenu.bind(
              null,
              c,
              i
            )}>
